...
Consolidated from all feature plans. Last updated: 2026-09-15
...
- T020–T027 are accepted: offline reviewed/automatic selection CLI, guarded SAM adapter and mask/alpha/glyph artifacts, native masked compilation and CPU composition. Core has no Torch dependency. Windows T024 verified the exact official SAM 2.1 snapshot on the cu130 environment with one approved fixed development sample, settled actual GPU time, and confirmed zero allocated CUDA bytes after release. T027 is accepted on macOS with pinned ComfyUI in an isolated letsaigc-comfy CPU environment, native-node/process verification, and no generation. T030 is accepted within the single-image MVP scope in docs/t030-mvp-scope.md; expanded editing and full quality acceptance remain pending. Windows models, ledger, evidence and approvals remain Windows-local and must not be assumed available on this Mac.
...
- T028 is accepted with offline providers and a real local Temporal restart test: root-owned sequential editing, exact segmentation/inpaint approvals, immutable review rebinding, confirmed GPU release, and bounded CPU reconstruction. Old parse history remains separate. T029 is accepted with bounded local revisions, immutable model suggestions and explicit human adoption, targeted SAM recomputation with unchanged asset reuse, and shared root costs/call limits. T030 delivered the optional cloud editing product chain with user-reviewed generated PNG; SC-017 live expansion, full local GPU editing/recovery coverage and formal quality remain pending. T031–T033 delivered sequential manual/search batches with isolated v5 ledger tests, shared root budgets, exact GPU approvals, cancellation/reconciliation boundaries, and derived manifest/MLflow lineage. T034 live batch acceptance is next; the actual Mac ledger is still v4.
