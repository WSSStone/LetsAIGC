...
- T028 is accepted with offline providers and a real local Temporal restart test: root-owned sequential editing, exact segmentation/inpaint approvals, immutable review rebinding, confirmed GPU release, and bounded CPU reconstruction. Old parse history remains separate. T029 is accepted with bounded local revisions, immutable model suggestions and explicit human adoption, targeted SAM recomputation with unchanged asset reuse, and shared root costs/call limits. T030 delivered the optional cloud editing product chain with user-reviewed generated PNG; SC-017 live expansion, full local GPU editing/recovery coverage and formal quality remain pending. Next is T031 sequential batch contracts; the actual Mac ledger is still v4.
