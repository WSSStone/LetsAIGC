...
已审查 Windows 提交 `b7aec34`，按上述单图 MVP 范围接收。独立代码复审未发现云端 child 批准、费用追加、unknown 保留及恢复路径的阻塞问题；Windows 真实结果承接本页现场记录与用户验收，不声称在 Mac 重跑或独立核验了未移交的现场证据。

集成修正：Comfy 的超限采样会清空稳定计数，只有连续两次相同的有界残留才能通过；非零总分配无法独立证明模型驻留为零，`model_cuda_allocated_bytes` 因而记录为 null，实际总字节及原有残留容限保留。另隔离三项单元测试对本机 Comfy checkout 和密钥的意外依赖，没有更改产品的凭据或版本校验。

回归用例先复现上述两项释放判断问题，修复后相关测试 23 passed。Mac 独立临时工作树完整验证 **802 passed、15 skipped、2 条既有 Pillow 警告**（99.02 秒），包含本地真实 Temporal 服务的离线恢复测试；ruff 和 diff check 通过。跳过项是未启用的现场验收与 Windows 专用测试，不能算作再次通过 T030/T024。日志位于 Mac `.local/validation/ui-analysis/t030-integration-20260915/`。首轮沙箱端口限制及缺少本地资源的失败未当作通过，最终全套在允许 loopback 服务并隔离测试依赖后重新完成。

Mac 主账本未迁移，未调用真实模型或 GPU。T030 按 MVP 完成，当前进度 38/50；下一项 T031 为顺序批次合同测试，可在 Mac 离线实现，再进入 T032 的批次串联。上文扩展现场验收及正式质量缺项继续保留。
