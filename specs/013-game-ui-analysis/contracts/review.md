...
2026-09-14 增量：可选云端补图复用本合同的已确认版本，不修改 review 数据结构或锁定规则。
VLM 补图提示词不是局部 review 建议，不能代替 SC-017 验收，不创建或确认新 review。
T030 优先[单图编辑 MVP](../../../docs/t030-mvp-scope.md)；其余真实局部复核属于保留的扩展待验收项。
