...
async def serve(config: TemporalConfig, root: Path):
    ...
    from .ui_batch_activities import UIBatchActivities
    from .ui_batch_workflow import UIBatchWorkflow
    ...
    batch_activities = UIBatchActivities(service)
    ...
    with ThreadPoolExecutor(max_workers=config.max_concurrent_activities) as executor:
        async with Worker(
            ...
            workflows=[SmokeWorkflow, ComfyGenerationWorkflow, UIAnalysisWorkflow, UIEditingWorkflow, UIBatchWorkflow],
            activities=[*activities.registered(), *ui_activities.registered(), *editing_activities.registered(),
                        *batch_activities.registered()],
